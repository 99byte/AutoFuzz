        ...
        try {
            ...
            if (response.ok) {
                ...
                if (onTaskCreated) {
                    ...
                    // Fallback if no callback provided, though usually the parent will handle refresh
                    // or we can redirect to the new task
    ...
    return (
        <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent side="right" className="sm:max-w-[500px] overflow-y-auto">
                <SheetHeader>
                    <SheetTitle>创建Fuzz任务</SheetTitle>
                    <SheetDescription>
                        AI将自动生成UI交互测试用例。配置完成后点击创建即可开始。
                    </SheetDescription>
                </SheetHeader>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 mt-6">
                    ...
                    {/* 测试深度 */}
                    <div className="space-y-2">
                        <Label htmlFor="testDepth">测试深度（用例数量）</Label>
                        <Input
                            id="testDepth"
                            type="number"
